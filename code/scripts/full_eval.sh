...
BASE_CMD="python main.py --decay=1e-4 --lr=0.001 --layer=3 --seed=2020 --epochs=151 --bpr_batch=4096 \
...
declare -a TRIALS=(
	# #vanilla 
	# "comment=final/${DATASET}/vanilla normalize_users=0 beta=0 alpha=0"

	# #optimal recall from hyperparam
	# "comment=final/${DATASET}/ours normalize_users=0 beta=0 alpha=0.25"

	# #optimal recall from hyperparam where beta is not zero
	# "comment=final/${DATASET}/our-non-zero-beta normalize_users=0 beta=-0.5 alpha=0.75"

	# #optimal recall from hyperparam where alpha=0 and beta is not zero (only item norm)
	# "comment=final/${DATASET}/only-item normalize_users=0 beta=-0.5 alpha=0"

	#vanilla but normalize users
	"comment=final/${MODEL}/${DATASET}/only-users normalize_users=1 beta=0 alpha=1"
